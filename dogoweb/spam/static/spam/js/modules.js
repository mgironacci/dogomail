...
$(function(){
    ...
    var clickDTs = function (et, e, dt, indexes) {
        var dts =  $('#modulos-table').DataTable();
    ...
    // Servidores
    $('#modulos-table')
    .on('select.dt', function ( e, dt, type, indexes ) { clickDTs('select.modulo', e, dt, indexes); } )
    .on('deselect.dt', function ( e, dt, type, indexes ) { clickDTs('deselect.modulo', e, dt, indexes); } )
    .DataTable({
        responsive: true,
        serverSide: true,
        ajax: {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.form_is_valid) {
                    $("#popup-modal").modal('hide');
                    if (data.panel == 'modulo') {
                        $('#modulos-table').DataTable().ajax.reload();
                        $('#edit').hide();
                        $('#delete').hide();
                    }
                }
