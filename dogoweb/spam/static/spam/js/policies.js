...
$(function(){
    ...
    var clickDTs = function (et, e, dt, indexes) {
        var dts =  $('#politicas-table').DataTable();
    ...
    // Servidores
    $('#politicas-table')
    .on('select.dt', function ( e, dt, type, indexes ) { clickDTs('select.politica', e, dt, indexes); } )
    .on('deselect.dt', function ( e, dt, type, indexes ) { clickDTs('deselect.politica', e, dt, indexes); } )
    .DataTable({
        responsive: true,
        serverSide: true,
        ajax: {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.form_is_valid) {
                    $("#popup-modal").modal('hide');
                    if (data.panel == 'politica') {
                        $('#politicas-table').DataTable().ajax.reload();
                        $('#edit').hide();
                        $('#delete').hide();
                    }
                }
