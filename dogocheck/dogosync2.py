...
allconf.read(BASE_DIR+'/dogosync2.ini')
...
def busca_dogos():
    ...
    lcur = lcon.cursor()
    lcur.execute('select id, dirip4, CONVERT_TZ(ultvis,"+00:00","SYSTEM"), sqlusr, sqlpas from mail_dogomail where activo=1 and tipodm="dogo2"')
...
try:
    with PidFile('dogosync2', '/var/lock'):
